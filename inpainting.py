# from https://github.com/sail-sg/EditAnything/blob/main/utils/stable_diffusion_controlnet_inpaint.py
import inspect
...
from transformers import CLIPImageProcessor, CLIPTextModel, CLIPTokenizer, AutoImageProcessor, UperNetForSemanticSegmentation
...
class StableDiffusionControlNetInpaintMixingPipeline(StableDiffusionControlNetInpaintPipeline):
    def __call__(
        ...
        # 0. Default height and width to unet
        height, width = self._default_height_width(height, width, controlnet_conditioning_image)
        print('Running with alpha = {}...'.format(alpha_weight))
        # 1. Check inputs. Raise error if not correct
        self.check_inputs(
            prompt,
            image,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            #               + ((1 - alpha_weight) * latents + alpha_weight * init_masked_image_latents) * (
            #                       1 - mask_image)
...
def image_to_seg(image_processor, image_segmentor, image):
    pixel_values = image_processor(image, return_tensors="pt").pixel_values
    with torch.no_grad():
        outputs = image_segmentor(pixel_values)
    seg = image_processor.post_process_semantic_segmentation(outputs, target_sizes=[image.size[::-1]])[0]
    ...
    for label, color in enumerate(palette):
        color_seg[seg == label, :] = color
    color_seg = color_seg.astype(np.uint8)
    seg_image = PIL.Image.fromarray(color_seg)
    return seg_image
